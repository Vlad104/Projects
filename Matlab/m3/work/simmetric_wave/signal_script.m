    ...
    for i=0:1:I/2-1 %нарастание
        ...
        ZOND_2(i+1) = exp(-fi*l)*sin(2*pi*kk*i*f0 + pi*b*(i*kk)^2); %модель зондирующего сигнала
    end;    
    for i=I/2:1:I-1
        ZOND_1(i+1) = exp( fi*l)*sin(2*pi*kk*i*f0 - pi*b*(i*kk)^2); %модель зондирующего сигнала 
        ZOND_2(i+1) = exp(-fi*l)*sin(2*pi*kk*i*f0 - pi*b*(i*kk)^2); %модель зондирующего сигнала
    ...
    for i=0:1:I-1
        ...
        S_SM(i+1,2) = ZOND_2(i+1)*S1_RX(i+1); %модель сигнала с выхода смесителя
        S_SM(i+1,3) = ZOND_1(i+1)*S2_RX(i+1); %модель сигнала с выхода смесителя
        S_SM(i+1,4) = ZOND_2(i+1)*S2_RX(i+1); %модель сигнала с выхода смесителя
        S_SM(i+1,5) = ZOND_1(i+1)*S3_RX(i+1); %модель сигнала с выхода смесителя
        S_SM(i+1,6) = ZOND_2(i+1)*S3_RX(i+1); %модель сигнала с выхода смесителя
        S_SM(i+1,7) = ZOND_1(i+1)*S4_RX(i+1); %модель сигнала с выхода смесителя
