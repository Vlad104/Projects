    ...
    ZOND_SIN(1:25000) = 0;     %зондирующий сигнал
    ZOND_COS(1:25000) = 0;     %зондирующий сигнал
    S1_RX(1:25000) = 0;    %сигнал, принятый первой антенной
    S2_RX(1:25000) = 0;    %сигнал, принятый втрой антенной
    S1_SM(1:25000) = 0;    %сигнал на выходе смесителя 1
    S2_SM(1:25000) = 0;    %сигнал на выходе смесителя 2
    S1(1:256) = 0;         %сигнал 1 после АЦП
    S2(1:256) = 0;         %сигнал 2 после АЦП
    ...
    V(1) = -2*0.78; % скорость цели, м/с
    R(1) = 3*120; %дальность до цели, м
    ...
    for i=0:1:I-1
    	...
    	ZOND_SIN(i+1) = sin(freq); %модель зондирующего сигнала  
    	ZOND_COS(i+1) = cos(freq); %модель зондирующего сигнала 
    end;  
    ...
    for i=0:1:I-1
    	for z = 1:1:N
        	freq = 2*pi*(i+J(z))*kk*f0 + pi*b*((i+J(z))*kk)^2 + 2*pi*fd(z)*1*Tm;
        	S1_RX(i+1) = S1_RX(i+1) + exp(frx1*l)*exp(phi(z)*l)*sin(freq + (mu+sigma.*randn(1,1))) + (mu+sigma.*randn(1,1));
        	S2_RX(i+1) = S1_RX(i+1) + exp(frx2*l)*exp(phi(z)*l)*sin(freq + (mu+sigma.*randn(1,1))) + (mu+sigma.*randn(1,1));
    	end;
    end;
    
    %Сигналы на выходе смесителя
    for i=0:1:I-1
    	S1_SM_SIN = ZOND_SIN(i+1)*S1_RX(i+1);
    	S1_SM_COS = ZOND_COS(i+1)*S1_RX(i+1);
            
    	S2_SM_SIN = ZOND_SIN(i+1)*S2_RX(i+1);
    	S2_SM_COS = ZOND_COS(i+1)*S2_RX(i+1);
            
     	%S1_SM(i+1) = complex( sqrt( abs(S1_SM_SIN) + abs(S1_SM_COS) )); 
    	%S2_SM(i+1) = complex( sqrt( abs(S2_SM_SIN) + abs(S2_SM_COS) ));
