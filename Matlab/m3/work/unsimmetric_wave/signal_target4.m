function [S1, S2, Rd, Vd] = signal_target4(EQ64, EQ256)
    ...
    V(1) = 1*Vd; % скорость цели, м/с
    R(1) = 10*Rd; %дальность до цели, м
    phi_grad(1) = -11; %угол пеленга в градусах
    % ЦЕЛЬ 1. Параметры конец
     
    % ЦЕЛЬ 2. Параметры начало
    V(2) = 5*Vd; % скорость цели, м/с
    R(2) = 100*Rd; %дальность до цели, м
    phi_grad(2) = -33; %угол пеленга в градусах
    % ЦЕЛЬ 2. Параметры конец 
    
    % ЦЕЛЬ 3. Параметры начало
    V(3) = -1*Vd; % скорость цели, м/с
    R(3) = 9*Rd; %дальность до цели, м
    phi_grad(3) = -66; %угол пеленга в градусах
    % ЦЕЛЬ 3. Параметры конец  
    
    % ЦЕЛЬ 4. Параметры начало
    V(4) = -5*Vd; % скорость цели, м/с
    R(4) = 50*Rd; %дальность до цели, м
    phi_grad(4) = -88; %угол пеленга в градусах
    % ЦЕЛЬ 4. Параметры конец 
