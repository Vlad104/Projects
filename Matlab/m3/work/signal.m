function [S1, S2] = signal(EQ64, EQ256)
    ...
    for i=0:1:I-1
        for j = 0:1:FF-1
            ...
            S1_RX(i+1,j+1) = sin(2*pi*(i+J1)*kk + pi*b*((i+J1)*kk)^2 + 2*pi*fd1*j*Tm + (mu+sigma.*randn(1,1))) + RAND; %модель принятого сигнала ЦЕЛЬ 1
            S1_RX(i+1,j+1) = S1_RX(i+1,j+1) + sin(2*pi*(i+J2)*kk + pi*b*((i+J2)*kk)^2 + 2*pi*fd2*j*Tm + (mu+sigma.*randn(1,1))) + RAND; %модель принятого сигнала ЦЕЛЬ 2  
            S1_RX(i+1,j+1) = S1_RX(i+1,j+1) + sin(2*pi*(i+J3)*kk + pi*b*((i+J3)*kk)^2 + 2*pi*fd3*j*Tm + (mu+sigma.*randn(1,1))) + RAND; %модель принятого сигнала ЦЕЛЬ 3  
    ...
    for i=0:1:I-1
        ...
        S2_RX(i+1,j+1) = exp(l*fi2)*sin(2*pi*(i+1+J1)*kk + pi*b*((i+1+J1)*kk)^2 + 2*pi*fd1*(j+1)*Tm+RAND)+RAND; %модель принятого сигнала ЦЕЛЬ 1
        S2_RX(i+1,j+1) = S2_RX(i+1,j+1) + exp(l*fi2)*sin(2*pi*(i+1+J2)*kk + pi*b*((i+1+J2)*kk)^2 + 2*pi*fd2*(j+1)*Tm+RAND)+RAND; %модель принятого сигнала ЦЕЛЬ 2 
        S2_RX(i+1,j+1) = S2_RX(i+1,j+1) + exp(l*fi2)*sin(2*pi*(i+1+J3)*kk + pi*b*((i+1+J3)*kk)^2 + 2*pi*fd3*(j+1)*Tm+RAND)+RAND; %модель принятого сигнала ЦЕЛЬ 3 
