function [S1, S2] = signal(EQ64, EQ256)
    ...
    for i=0:1:I-1
        for j = 0:1:FF-1
        ZOND(i+1,j+1) = sin(2*pi*(5*10^-9)*(i+J)+pi*b*((i+J)*(5*10^-9))^2); %модель зондирующего сигнала
        S1_RX(i+1,j+1) = 1*(sin(2*pi*((i)*(5*10^-9))+pi*b*((i)*(5*10^-9))^2+2*pi*fd*j*Tm+(mu+sigma.*randn(1,1)))+RAND); %модель принятого сигнала
    ...
    for i=0:1:I-1
        ...
        S2_RX(i+1,j+1) = exp(l*fi2)*sin(2*pi*(i+1)*5*10^(-9)+pi*b*((i+1)*5*10^(-9))^(2)+2*pi*fd*(j+1)*Tm+RAND)+RAND; %модель принятого сигнала
                                                                                     %с доплеровской частотой и задержкой
